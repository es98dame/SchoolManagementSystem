...
class Alidb {
	...
	function queryAsSingle($sql, $type = MYSQL_ASSOC){
	...
	function queryAsArray($sql, $type = MYSQL_ASSOC){
	    $result = $this->query($sql); $this->isqueryresource($result);
	    $data = array();
	...
	/* turns your result into an array (valid typs are MYSQL_ASSOC, MYSQL_NUM, and MYSQL_BOTH) */
	function queryGetHolidays($pClassGropuNo,$items,$type = MYSQL_ASSOC){
	...
	function queryGetCalendar($auth,$permis,$type = MYSQL_ASSOC){
		$sql = "select a.calboard_no, a.instructors_no,b.authority,b.firstname,b.name,b.initial,b.nickname,b.bgcolorone, a.subject, a.contents, a.thisday, a.state, a.regdate from ali_calboard a, ali_instructors b where a.permission in ('".$permis."') and a.instructors_no=b.instructors_no and b.authority in (".$auth.") order by a.thisday";
	    $result = $this->query($sql); $this->isqueryresource($result);
		 $cntin=0; $preday="";
		while($row = mysqli_fetch_array($result, $type)):
		      $subj = (strlen($row['subject'])>25)?substr($row['subject'],0,25)."...":$row['subject'];
			    if($row['authority']==3):  $subj = "<font color=\"#008000\">".$subj."</font>"; endif;
				if($row['state']=='C'):  $subj = "<s>".$subj."</s>"; endif;
				if($permis=='P'): //Personal
					$indata[ $row['thisday'] ][]= "<span title='".$row['firstname']." ".$row['name'].": ".$row['contents']."'> <span style='background-color: ".$row['bgcolorone'].";'><b>".$row['initial']."</b></span>: ".$subj." </span>";
				else: //Shared
	...
	function queryGetStaff($staff,$type = MYSQL_ASSOC){
		$sql = "select instructors_no,firstname,name from ali_instructors where status='A'";
	    $result = $this->query($sql); $this->isqueryresource($result);
		$contents = "<option value=\"\">Staff</option>";
		while($row = mysqli_fetch_array($result, $type)):
		 	if($staff==$row['instructors_no']): $sel="selected"; else: $sel=""; endif;
			$contents .= "<option value=\"".$row['instructors_no']."\" ".$sel.">".$row['firstname']." ".$row['name']."</option>";
		endwhile;
	   return $contents;
	}
	...
	function GetStudentRank($vStudentsNo,$pStartday,$pEndday,$pClassGropuNo){
			$result8 = $this->queryAsArray("select a.GUBUN, a.class_no, a.items,a.period2, count(a.items) as itemscnt, d.class_name, e.name AS instructorname  from ali_attendance a, ali_enroll_class b, ali_students c, ali_classes d, ali_instructors e  where a.students_no=b.students_no and  a.students_no=c.students_no and  c.students_no=b.students_no and a.students_no=".$vStudentsNo." AND a.class_no = d.classes_no AND a.instructors_no = e.instructors_no and a.semester_no=b.semester_no and a.semester_no=".$pClassGropuNo." AND a.attendance_day BETWEEN b.startday AND b.endday group by c.name, a.GUBUN, a.semester_no, a.class_group_no,a.period2, a.class_no, a.items ");
