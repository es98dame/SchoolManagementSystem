...
<script>
    ...
    dhxLayout.cells("a").setText("<div style=\"float:left;font-family: Tahoma; font-size: 15px; font-weight: bold; color: #696969;\">Attendance Sheet</div>");
    ...
    gpclassfm.attachEvent("onButtonClick", function(name,command){
        if(gpclassfm.validate()){
            if(name=='btSearch'){
                ...
                subGrid.clearAndLoad("<?php echo $_SERVER["PHP_SELF"];?>/getStudentList?unqueId="+(new Date()).valueOf()+oparams,function(){
                });
    ...
    var mainfmData = [
        {type: "fieldset",  name: "emydata", label: "Search Results",offsetLeft:10, width:"auto", list:[
            ...
            {type: "combo", label: "Class: ",inputWidth:140,required: true, name: "classtype", options:[
                {text: "", value: "", selected: true},
                {text: "LS", value: "LS"},
                {text: "RW", value: "RW"}
            ] },
    ...
    function showNewWindow(){
        ...
        var classtype = mainfm.getCombo("classtype").getSelectedValue();
        ...
        sheetgrid.setHeader("ALI Class Attendance Sheet,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan",null,["text-align:center"]); //,null,["text-align:left","text-align:left","text-align:left","text-align:left"]
        ...
        sheetgrid.attachHeader("Instructor,#cspan,"+teacher+",#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,Course,#cspan,#cspan,#cspan,Level "+level+" "+classtype+",#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,Session,#cspan,#cspan,#cspan,"+v5+",#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan"); //#cspan
        ...
        sheetgrid.setSizes();
        sheetgrid.init();
        sheetgrid.clearAndLoad("<?php echo $_SERVER["PHP_SELF"];?>/getAttSheet?vyear="+v1+"&vtrim="+v2+"&vlevel="+v4+"&vsession="+session+"&vmon="+mon+"&vclasstype="+classtype+"&ucnt="+uncnt+"&unqueId="+(new Date()).valueOf(),function(){
            var rowID=sheetgrid.getRowId(0);
            sheetgrid.cells(rowID,43-uncnt).setValue(memo);
        });

        w1.setText("Attendance Sheet");
        w1.setModal(false);
        w1.show();
    }
    ...
    function exportAttSheet(){
        ...
        var classtype = mainfm.getCombo("classtype").getSelectedValue();
        var memo = mainfm.getItemValue("memo");
        var trima= v2 + " Trimester " + v1 + " " + month;

        document.getElementById("id_trima").value = trima;
        document.getElementById("id_teacher").value = teacher;
        document.getElementById("id_vyear").value = v1;
        document.getElementById("id_vtrim").value = v2;
        document.getElementById("id_vlevel").value = v4;
        document.getElementById("id_vsession").value = session;
        document.getElementById("id_tsession").value = v5;
        document.getElementById("id_vmon").value = mon;
        document.getElementById("id_vclasstype").value = classtype;

        document.exportform.action = "attsheet/exportAttSheet";
        document.exportform.submit();
    }
</script>

<form action="" target="export_area"  method="post" name="exportform" id="exportform">
    <input type="hidden" id="id_trima" name="trima"/>
    <input type="hidden" id="id_teacher" name="teacher"/>
    <input type="hidden" id="id_vyear" name="vyear"/>
    <input type="hidden" id="id_vtrim" name="vtrim"/>
    <input type="hidden" id="id_vlevel" name="vlevel"/>
    <input type="hidden" id="id_vsession" name="vsession"/>
    <input type="hidden" id="id_tsession" name="tsession"/>
    <input type="hidden" id="id_vmon" name="vmon"/>
    <input type="hidden" id="id_vclasstype" name="vclasstype"/>
</form>
