...
<div id="SubMenuId" style="margin-left:-20px;"><ul style="list-style:none;"><?php echo $param["menus"];?></ul></div>
    ...
    <script>
        ...
        function loadClassTeachers(){
            ...
            discgrid.clearAndLoad("<?php echo $_SERVER["PHP_SELF"];?>/getClassTeachers?unqueId="+(new Date()).valueOf()+ oparams,function(){
                dhxLayout.cells("b").progressOff();
            });
        }

        var SendformData = [    {type: "settings", position: "label-left",offsetLeft:25,offsetTop:5, labelWidth: 80,inputWidth: "auto"},
            {type: "combo",  className: "css_form1", label: "Teachers: ",inputWidth:200, name: "teacher_no",id: "teacher_no", connector:"<?php echo $_SERVER["PHP_SELF"];?>/getComClassTeachers?mod=insert&unqueId="+(new Date()).valueOf(), required: true },
        ...
        var	dpTeacher = new dataProcessor("<?php echo $_SERVER["PHP_SELF"];?>/setClassTeacher?unqueId="+(new Date()).valueOf()); //lock feed url
        ...
        dpTeacher.defineAction("updated",function(response){
            discgrid.updateFromXML("<?php echo $_SERVER["PHP_SELF"];?>/getClassTeachers?unqueId="+(new Date()).valueOf(),true,true,function(){
                w2.setModal(false);
                w2.hide();
        ...
        function showNewWindow(){
            ...
            combo1.setComboText("");
            w2.progressOn();
            combo1.load("<?php echo $_SERVER["PHP_SELF"];?>/getComClassTeachers?mod=insert&classno=<?php echo $param["classno"];?>&teacher_no=&unqueId="+(new Date()).valueOf(),function(){
                w2.progressOff();
                this.setComboValue("");
            });
        ...
        function doOnLoadInfo(sno,tno,pno){
            ...
            combo2.setComboText("");
            w2.progressOn();
            combo2.load("<?php echo $_SERVER["PHP_SELF"];?>/getComClassTeachers?mod=update&classno=<?php echo $param["classno"];?>&teacher_no="+tno+"&unqueId="+(new Date()).valueOf(),function(){
                w2.progressOff();
            ...
            editForm2.load("<?php echo $_SERVER["PHP_SELF"];?>/getClassTeacher?id="+sno+"&unqueId="+(new Date()).valueOf());
        ...
        function myCallBack(message) {
            ...
            discgrid.updateFromXML("<?php echo $_SERVER["PHP_SELF"];?>/getClassTeacher?unqueId="+(new Date()).valueOf(),true,true,function(){
                w2.setModal(false);
                w2.hide();
            });
        }
