...
<script>
  //  window.onload=loadRoster;
    ...
    dhxLayout.cells("a").setText("<div style=\"float:left;font-family: Tahoma; font-size: 15px; font-weight: bold; color: #696969;\">School Roster</div>");
    ...
    gpclassfm.attachEvent("onButtonClick", function(name,command){
        if(gpclassfm.validate()){
            if(name=='btSearch'){
                ...
                grid_classgrid.clearAndLoad("<?php echo $_SERVER["PHP_SELF"];?>/getRoster?unqueId="+(new Date()).valueOf()+ oparams,function(){
            ...
            if(name=='btExcel'){
                ...
                grid_classgrid.toExcel('http://'.$_SERVER['HTTP_HOST'].'/dhtmlxSuite/sources/dhtmlxGrid/codebase/grid-excel-php/generate.php?title='+v6+'&filename=SchoolRoster'+v6);
                return true;
            }
            if(name=='btPDF'){
