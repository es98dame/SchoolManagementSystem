...
<script>
    window.onload = loadStaffs;
        ...
        dhxLayout.cells("a").setText('Staff Info');
        dhxLayout.cells("b").setText('<input type="button" name="save" value="Export(.xlsx)" style="font-size: 11px; font-family: Tahoma;" onClick="staffgrid.toExcel(\'<?php echo base_url("dhtmlxSuite/sources/dhtmlxGrid/codebase/grid-excel-php/generate.php"); ?>\')"> <input type="button" name="save" value="Export(.pdf)" style="font-size: 11px; font-family: Tahoma;" onClick="staffgrid.toPDF(\'<?php echo base_url("dhtmlxSuite/sources/dhtmlxGrid/codebase/grid-pdf-php/generate.php"); ?>\')">');
        dhxLayout.cells("a").setWidth(380);

        var staffgrid =dhxLayout.cells("b").attachGrid();
        staffgrid.setImagePath("dhtmlxSuite/sources/dhtmlxGrid/codebase/imgs/");
        staffgrid.setHeader("Authority,User ID,FirstName,LastName,Initial,Nickname,Cellphone,Email,Color,Status,Writer,Write date,Memo,&nbsp;");
        staffgrid.setColumnIds("roleid,user_ID,firstname,lastname,initial,nickname,cellphone,email,bgcolorone,active,writer,created,etc,passw");
        staffgrid.setInitWidths("80,80,80,80,50,80,100,100,50,50,80,150,0,0");
        staffgrid.enableAutoWidth(true);
        staffgrid.setColAlign("left,left,left,left,left,center,center,center,center,center,center,center,left,left");
        staffgrid.setColTypes("co,ed,ed,ed,ed,ed,ed,ed,ed,co,txt,ro,ro,ro");
        staffgrid.setColSorting("str,str,str,str,str,str,str,str,str,str,str,str,date,str");
        staffgrid.getCombo(0).put('2', "Manager");
        staffgrid.getCombo(0).put('4', "Receptionist");
        staffgrid.getCombo(9).put('1', "Active");
        staffgrid.getCombo(9).put('0', "Inactive");
        staffgrid.setSkin("dhx_skyblue");
        staffgrid.attachEvent("onRowSelect", function(id,ind){
            myFormAdd.enableItem("button_save");
            myFormAdd.enableItem("button_delete");
            myFormAdd.disableItem("button_insert");
        });
        staffgrid.attachEvent("onBeforeSorting",function(ind,type,direction){
            window.sf_col = ind;
            window.sf_drt = ((direction == "des") ? "asc": "des");
            return true;
        });
        staffgrid.init();
        staffgrid.enableSmartRendering(true, 50);

        var myDataProcessor = new dataProcessor("staffs/setStaff?unqueId="+(new Date()).valueOf()); //lock feed url
        //myDataProcessor.setTransactionMode("POST",true); //set mode as send-all-by-post
        myDataProcessor.init(staffgrid); //link dataprocessor to the grid
        myDataProcessor.defineAction("inserted",function(response){
            ...
            staffgrid.updateFromXML("staffs/getStaffs?unqueId="+(new Date()).valueOf());
            return true;
        });
        ...
        var formData = [{ type: "settings", position: "label-left", labelWidth: 100},
            {type: "fieldset",label: "Insert Data",inputWidth: "auto", list: [
                ...
                {type: "select", label: "Authority",name: "roleid", required: true, options: [ {value: "2",text: "Manager", selected:true }, { value: "4",text: "Receptionist"}] },
        ...
        myFormAdd.attachEvent("onButtonClick", function(name,command){
            ...
            if(name=='button_delete'){
                var selectedId=staffgrid.getSelectedRowId();
                if(selectedId){
                    if (confirm("Are you sure you want to delete row")) {
                        staffgrid.deleteSelectedItem();
                        myFormAdd.clear();
                        return true;
                    }else{
                        return false;
            ...
            if(myFormAdd.validate()){
                ...
                if(name=='button_insert'){
                    ...
                    dhx4.ajax.post("staffs/checkDuplicateUserID","newuid="+uid, function(r){
                        ...
                        if(result>0){
                            ...
                            staffgrid.addRow(newId,[autho,uid,fnam,nam,initi,nicnam,cephon,emai,bgcolo,sta,'',getfullday(),etc,pwd],0);
                            setdefaultform();
                            return true;
                        }
                    });
    ...
    function loadStaffs(){
        ...
        var oparams = "";
        if(window.sf_drt){
            staffgrid.setSortImgState(true, window.sf_col, window.sf_drt,1);
            oparams="&orderby=" + window.sf_col + "&direct=" + window.sf_drt;
        }

        dhxLayout.cells("b").progressOn();
        staffgrid.clearAndLoad("staffs/getStaffs?unqueId="+(new Date()).valueOf()+ oparams,function(){
    ...
    function setdefaultform(){
        ...
        myFormAdd.setItemValue("roldid",2);
