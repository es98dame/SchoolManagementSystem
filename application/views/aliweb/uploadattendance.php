...
<div id="files_container">
    <form action="uploadattendance/setImportFile2" method="post" enctype="multipart/form-data" name="realForm" id="realForm" target="upload_area"><div id="dhxForm"></div></form>
</div>
...
<script>
    ...
    var formData = [{ type: "settings", position: "label-left", labelWidth: 100, inputWidth: 200},
        {type: "fieldset",label: "Import Files", inputWidth: "auto", list: [
            {type: "combo",  label: "Trimester : ", name: "trimester_no", id:"trimester_no", connector: "uploadattendance/getTrimestersCombo",required: true },
            ...
            case "import":
                if(selectedId){
                    if (confirm("Are you sure you want to import temp data")) {
                        var posturl = "uploadattendance/setImportTemps2", params = "fno="+selectedId+"&gno="+gno;
    ...
    myfamilyMenu.loadStruct("uploadattendance/menucontextUploadAttendace?unqueId="+(new Date()).valueOf());
    ...
    function loadFiles(gno){
        ...
        filesgrid.clearAndLoad("uploadattendance/getImportFiles2?gno="+gno+"&unqueId="+(new Date()).valueOf()+ oparams,function(){
            //loadclassall(gno);
            //loadstudentid();
        });
    }

    var	myDataProcessor = new dataProcessor("uploadattendance/setImportFiles2?unqueId="+(new Date()).valueOf());
    ...
    myDataProcessor.defineAction("updated",function(response){
        ...
        filesgrid.updateFromXML("uploadattendance/getImportFiles2?unqueId="+(new Date()).valueOf());
        return true;
    });
    ...
    datagrid.setHeader("No,ClassId,Year,GP,ClassName,First,Last,StudentId,Date,Mark,Writer,Created");
    datagrid.attachHeader("&nbsp;,#text_filter,#select_filter,#select_filter,#select_filter,#text_filter,#text_filter,#text_filter,#select_filter,#select_filter,&nbsp;,&nbsp;");
    datagrid.setColumnIds("file_no,engradeclassid,classschoolyear,classgradingperiod,classname,studentfirst,studentlast,studentid,attendancedate,mark,writer,regdate");
    datagrid.setInitWidths("30,100,40,30,100,70,70,100,80,60,60,100");
    datagrid.enableAutoWidth(true);
    datagrid.setColAlign("left,left,left,left,left,left,left,left,left,left,left,left");
    datagrid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro");
    datagrid.setColSorting("int,str,str,str,str,str,str,str,str,str,str,date");
    ...
    function loadData(fno){
        ...
        if(fno){
            ...
            dhxLayout.cells("b").progressOn();
            datagrid.clearAndLoad("uploadattendance/getEGAttendances?fno="+fno+"&unqueId="+(new Date()).valueOf()+ oparams,function(){
                dhxLayout.cells("b").progressOff();
            });
        }
    }
/*
    var	myDataProcessor2 = new dataProcessor("uploadattendance/setImportData?unqueId="+(new Date()).valueOf()); //lock feed url
    ...
    function setData(menuitemId) {
        ...
        if(totalrows>0){
            ...
            if (confirm(question)) {
                var posturl = "uploadattendance/setUpdateData.php", params = "mode="+menuitemId+"&fno="+fno;
    ...
    function loadclassall(sno){
        ...
        classdata.clearAll();
        classdata.load("uploadattendance/getClasswDV.php?gno="+sno,function(){
    ...
    function loadstudentid(){
        ...
        stdiddata.clearAll();
        stdiddata.load("uploadattendance/getStudentIDwDV.php",function(){
            addcombos(combo1,this);
        });
    }
